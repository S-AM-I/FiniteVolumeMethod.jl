...
@inline eval_condition_fnc(
    prob::AbstractFVMProblem, fidx, x, y, t,
    u
) = eval_condition_fnc(prob.conditions, fidx, x, y, t, u)
@inline eval_source_fnc(
    prob::AbstractFVMProblem, x, y, t,
    u
...
"""Return the coordinate system (e.g. `Cartesian`, `Cylindrical`) associated with the problem's mesh."""
...
The returned value is the corresponding [`FVMProblem`](@ref) struct. You can then solve the problem using [`solve(::Union{FVMProblem,FVMSystem}, ::Any; kwargs...)`](@ref CommonSolve.solve(::Union{FVMProblem,FVMSystem}, ::Any; kwargs...)) from DifferentialEquations.jl.
"""
...
function Base.show(io::IO, ::MIME"text/plain", prob::FVMProblem)
    ...
    tf = prob.final_time
    return print(io, "FVMProblem with $(nv) nodes and time span ($t0, $tf)")
end
@inline eval_flux_function(
    prob::FVMProblem, x, y, t, α, β,
    γ
) = prob.flux_function(x, y, t, α, β, γ, prob.flux_parameters)

function FVMProblem(
        mesh::FVMGeometry, boundary_conditions::BoundaryConditions,
        internal_conditions::InternalConditions = InternalConditions();
        diffusion_function = nothing,
        diffusion_parameters = nothing,
    ...
    @assert length(initial_condition) ==
        DelaunayTriangulation.num_points(mesh.triangulation) "The initial condition must have the same number of elements as the number of nodes in the mesh (including nodes that aren't vertices in the mesh itself)."
...
function FVMProblem(
        mesh::FVMGeometry, conditions::Conditions;
        diffusion_function = nothing,
        diffusion_parameters = nothing,
        source_function = (x, y, t, u, p) -> zero(eltype(u)),
        source_parameters = nothing,
...
You can then solve the problem using [`solve(::SteadyFVMProblem, ::Any; kwargs...)`](@ref CommonSolve.solve(::SteadyFVMProblem, ::Any; kwargs...)) from NonlinearSolve.jl. Note that you
...
@inline eval_flux_function(
    prob::SteadyFVMProblem, x, y, t, α, β,
    γ
) = eval_flux_function(prob.problem, x, y, t, α, β, γ)
...
struct FVMSystem{N, FG, P, IC, FT, F, S, FF} <: AbstractFVMProblem
    ...
    flux_functions::FF
    function FVMSystem(
            mesh::FG, problems::P, initial_condition::IC, initial_time::FT,
            final_time::FT, conditions, num_fncs, functions::F, source_functions::S,
            flux_functions::FF
        ) where {FG <: FVMGeometry, P, IC, FT, F, S, FF}
        ...
        @assert size(initial_condition) ==
            (length(problems), length(problems[1].initial_condition)) "The initial condition must be a matrix with the same number of rows as the number of problems."
        @assert all(
            i -> problems[i].conditions.neumann_edges == conditions[i].neumann_edges, 1:length(problems)
        ) "The Neumann edges in the `i`th `SimpleConditions` must match those from the `i`th problem."
        @assert all(
            i -> problems[i].conditions.constrained_edges ==
                conditions[i].constrained_edges,
            1:length(problems)
        ...
        sys = new{length(problems), FG, P, IC, FT, F, S, FF}(
            mesh, problems, initial_condition, initial_time, final_time,
            conditions, num_fncs, functions, source_functions, flux_functions
        )
...
function Base.show(io::IO, ::MIME"text/plain", prob::FVMSystem{N}) where {N}
    return print(io, "FVMSystem with $N equations and time span ($(prob.initial_time), $(prob.final_time))")
end
...
@inline get_neumann_fidx(prob::FVMSystem, i, j, var) = get_neumann_edges(get_conditions(prob, var))[
    (
        i, j,
    ),
]
@inline get_dirichlet_fidx(prob::FVMSystem, i, var) = get_dirichlet_nodes(get_conditions(prob, var))[i]
@inline get_constrained_fidx(
    prob::FVMSystem, i, j, var
) = get_constrained_edges(get_conditions(prob, var))[
    (
        i, j,
    ),
]
...
@inline eval_condition_fnc(
    ...
    u
) = eval_fnc_in_het_tuple(prob.functions, map_fidx(prob, fidx, var), x, y, t, u)
@inline eval_source_fnc(
    prob::FVMSystem, var, x, y, t,
    u
...
@inline eval_flux_function(
    prob::FVMSystem, x, y, t, α, β,
    γ
) = eval_all_fncs_in_tuple(prob.flux_functions, x, y, t, α, β, γ)
...
function FVMSystem(probs::Vararg{FVMProblem, N}) where {N}
    ...
    conditions, num_fncs, fncs, source_functions,
        flux_functions = merge_problem_conditions(probs)
    return FVMSystem(
        mesh, probs, initial_condition, initial_time, final_time,
        conditions, num_fncs, fncs, source_functions, flux_functions
    )
end
