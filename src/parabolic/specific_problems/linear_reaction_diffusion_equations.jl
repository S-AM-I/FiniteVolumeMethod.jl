...
function Base.show(io::IO, ::MIME"text/plain", prob::LinearReactionDiffusionEquation)
    ...
    tf = prob.final_time
    return print(io, "LinearReactionDiffusionEquation with $(nv) nodes and time span ($t0, $tf)")
end

function LinearReactionDiffusionEquation(
        mesh::FVMGeometry,
        BCs::BoundaryConditions,
        ICs::InternalConditions = InternalConditions();
        diffusion_function,
    ...
    return LinearReactionDiffusionEquation(
        mesh, conditions,
