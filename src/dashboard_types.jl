...
# Variable names for each conservation law moved to
# src/hyperbolic/variable_names.jl (Stage 3e): every method dispatches on a
# conservation-law type owned by the Hyperbolic module, which now owns the
# generic. This file (and core/symbolic_indexing.jl) consume it through the
# layer-file `import .Hyperbolic: variable_names` guard.
