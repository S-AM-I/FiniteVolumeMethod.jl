...
- Nine collocated features are back at `provisional` (re-promoted in v4.0.0-DEV) with machine-linked `[[scientific_evidence]]` entries under `docs/src/literate_verification/` executed by `validation/evidence_runner.jl`: `incompressible_ns` (Poiseuille O(h²) + Ghia 1982 cavity), `turbulence_rans` (DHIT exact-ODE decay + log-layer P_k = ε equilibrium), `conjugate_heat_transfer` (Laplace-series conduction + separable unsteady-heat decay), `turbulence_les` (Smagorinsky shear algebra + WALE operator invariants), `multiphase_vof` (kinematic disc-translation invariants + MULES/FCT limiter guarantees), `radiation` (P1 cold-medium slab attenuation), `collocated_operators` (Laplacian MMS + gradient/divergence MMS), `lagrangian_dpm` (Stokes settling ODE), `dynamic_mesh` (ALE GCL round-off exactness), `linear_solver_infra` (backend parity on reference Poisson), `postprocessing` (closed-form vorticity/Q/enstrophy on linear flows), `polyhedral_mesh_io` (closed-form polyhedral volume primitives), `combustion` (Arrhenius closure algebra + Magnussen-Hjertager EDM invariants) — **all 13 features demoted in v3.112 are now re-promoted**. The governance ladder gate (enforced for `provisional` as well as `stable` in `test/governance/repository_governance.jl`) passes with machine-linked evidence for every collocated feature; `stable` promotion still requires ≥3 published benchmarks per feature in CI
