...
- `incompressible_ns` and `turbulence_rans` are back at `provisional` (re-promoted in v4.0.0-DEV) with machine-linked `[[scientific_evidence]]` entries under `docs/src/literate_verification/` executed by `validation/evidence_runner.jl`: Poiseuille O(h²) convergence + Ghia 1982 cavity for `incompressible_ns`; DHIT exact-ODE decay + log-layer P_k = ε equilibrium for `turbulence_rans`. The remaining 11 collocated features are still `experimental` (demoted from `provisional` in v3.112): their "Evidence #N" items are real `test/` files run by `Pkg.test()`, but none are machine-linked `[[scientific_evidence]]` entries, so the governance ladder gate — enforced for `provisional` as well as `stable` in `test/governance/repository_governance.jl` — is not satisfied. Re-promotion follows the same evidence-script pattern
