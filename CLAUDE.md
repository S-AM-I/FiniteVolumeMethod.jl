...
- `incompressible_ns` is back at `provisional` (re-promoted in v4.0.0-DEV): machine-linked `[[scientific_evidence]]` entries `evidence-poiseuille-collocated` (verification: O(h²) exact-solution grid convergence) and `evidence-ghia-cavity-re100` (benchmark: Ghia 1982 Table I centerline, Re = 100) live under `docs/src/literate_verification/` and are executed by `validation/evidence_runner.jl`. The remaining 12 collocated features are still `experimental` (demoted from `provisional` in v3.112): their "Evidence #N" items are real `test/` files run by `Pkg.test()`, but none are machine-linked `[[scientific_evidence]]` entries, so the governance ladder gate — enforced for `provisional` as well as `stable` in `test/governance/repository_governance.jl` — is not satisfied. Re-promotion follows the same pattern as `incompressible_ns`
