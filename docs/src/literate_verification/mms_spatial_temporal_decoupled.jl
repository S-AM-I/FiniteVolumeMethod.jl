...
fig1
@test_reference joinpath(@__DIR__, "../figures", "mms_decoupled_convergence.png") fig1 #src

# ## Test Assertions
# Part A: Spatial rates should be approximately second order.
@test minimum(rates_A) > 1.5 #src
# Part B: Errors should decrease monotonically with tighter tolerances.
@test all(errors_B[i] >= errors_B[i + 1] for i in 1:(length(errors_B) - 1)) #src
