...
for (i, j) in zip(1:3, (1, 3, 5))
    ...
    tricontourf!(
        ax, tri, sol.u[j], levels = 0:0.005:0.05, colormap = :matter, extendhigh = :auto
    )
    tightlimits!(ax)
end
...
for (i, j) in zip(1:3, (1, 3, 5))
    ...
    tricontourf!(
        ax, tri, sol.u[j], levels = 0:0.05:1, extendlow = :auto,
...
You can view the source code for this file [here](https://github.com/cx-xd/FiniteVolumeMethod.jl/tree/main/docs/src/literate_tutorials/porous_medium_equation.jl).
