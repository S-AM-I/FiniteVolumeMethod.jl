...
sol = solve(
    prob, TRBDF2(linsolve = KLUFactorization()), saveat = LogRange(
        1.0e2, prob.final_time, 24 *
            10
    )
)
...
fig, ax,
    sc = tricontourf(
    tri, ∇norms, colormap = :matter, levels = LinRange(0, 0.0035, 25),
...
You can view the source code for this file [here](https://github.com/cx-xd/FiniteVolumeMethod.jl/tree/main/docs/src/literate_tutorials/solving_mazes_with_laplaces_equation.jl).
