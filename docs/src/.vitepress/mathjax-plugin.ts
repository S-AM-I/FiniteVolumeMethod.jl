...
// mathjax-plugin.ts
// Local override of DocumenterVitepress's default mathjax-plugin.ts —
// adds the `physics` extension so macros like \pdv, \vb, \div, \grad, \curl render.
// @ts-ignore
...
async function initializeMathJax(options: MathJaxOptions = {}) {
  ...
  const config: any = {
    ...
    tex: {
      tags: 'ams',
      packages: {
        '[+]': ['boldsymbol', 'braket', 'mathtools', 'physics'],
      },
    },
    output: {
      font,
