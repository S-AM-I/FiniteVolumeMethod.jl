...
**Problem:** FormatCheck, Nightly, Release, TagBot workflows are disabled (`.yml.disabled`). Re-enable plan documented in `validation/CI_REENABLE_PLAN.md`.

**Changes:**
- Follow the staged criteria in `CI_REENABLE_PLAN.md`:
  1. Verify local lanes routinely green (already true per docs)
  2. Verify v2.0.0-rc1 dry run completed (already true per docs)
  3. Rename `.yml.disabled` -> `.yml` for each workflow
  4. Start with FormatCheck (lowest risk — just checks Runic formatting)
  5. Then Nightly (runs full test suite nightly)
  6. Then Release + TagBot (only after first accepted release candidate)
- Verify each workflow passes after re-enabling

**Files:** `.github/workflows/FormatCheck.yml.disabled`, `Nightly.yml.disabled`, `Release.yml.disabled`, `TagBot.yml.disabled`

**Acceptance:** All 4 workflows are active and passing (or appropriately gated for Release/TagBot).
...
Then verify CRUD.jl workspace:
```bash
cd /home/sami/Code/workspaces/1/CRUD.jl
julia --project=. -e 'using CRUDApplication; println("CRUD loads successfully")'
```
