...
**Status: shipped.** Cloud Actions are live (CI, Docs, Nightly, FormatCheck, benchmarks, docs-quality, jet); only `Release.yml.disabled` remains gated. The staged re-enable plan that informed this task has been retired (commit `e909f39`). The substeps below are preserved as a historical record of how the re-enable was sequenced.
...
All re-enable preconditions met (local lanes green, RC1 dry run done).
